...
// Filtros atuais
$tamanhosSelecionados = $_GET['tamanho'] ?? []; // array de tamanhos

$stock = $_GET['stock'] ?? '';
$ordenar = $_GET['ordenar'] ?? '';
$preco_min = $_GET['preco_min'] ?? '';
$preco_max = isset($_GET['preco_max']) && is_numeric($_GET['preco_max']) ? (float)$_GET['preco_max'] : 9999;

$tamanhos = obterTamanhosDisponiveis($liga); // Continua a vir da base de dados
// Preço máximo real dos produtos
$preco_maximo_real = mysqli_fetch_assoc(mysqli_query($liga, "SELECT MAX(preco) AS maximo FROM produtos"))['maximo'] ?? 500;
$preco_maximo_real = ceil($preco_maximo_real); // arredonda para cima


// Obter produtos com paginação

$produtos = listarProdutosComFiltros($liga, $pagina_atual, $produtos_por_pagina, $tamanhosSelecionados, $stock, $ordenar, $preco_max);
$total_produtos = contarProdutosComFiltros($liga, $tamanhosSelecionados, $stock);


// Total de produtos
// Corrigir total de produtos filtrados
$total_produtos = contarProdutosComFiltros($liga, $tamanhosSelecionados, $stock);
...
<script>
    ...
    items.forEach(item => {
        ...
        const caminhoImagemHover = item.getAttribute('caminho_imagem_hover');
